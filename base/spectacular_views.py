...
def serve_swagger_file(request, filename):
    ...
    if filename == 'swagger-ui.css':
        ...
        if not file_path.exists():
            response = requests.get('https://cdn.jsdelivr.net/npm/swagger-ui-dist@5.9.0/swagger-ui.css')
        ...
        if not file_path.exists():
            response = requests.get('https://cdn.jsdelivr.net/npm/swagger-ui-dist@5.9.0/swagger-ui-bundle.js')
            with open(file_path, 'wb') as f:
                f.write(response.content)
        ...
        if not file_path.exists():
            response = requests.get('https://cdn.jsdelivr.net/npm/swagger-ui-dist@5.9.0/swagger-ui-standalone-preset.js')
            with open(file_path, 'wb') as f:
                f.write(response.content)
