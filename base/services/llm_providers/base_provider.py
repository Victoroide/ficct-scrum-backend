...
class BaseLLMProvider(ABC):
    ...
    def __init__(self, model_type: ModelType):
        ...
        logger.info(f"[{self.provider_name.upper()}] Initialized provider for model: {self.model_name}")
    ...
    def _validate_response(self, response: LLMResponse) -> bool:
        ...
        if len(response.content.strip()) < 3:
            logger.warning(f"[{self.provider_name.upper()}] Response too short: {len(response.content)} chars")
            return False
        ...
        if len(words) > 10:
            ...
            if max_repetition > len(words) * 0.5:  # More than 50% repetition
                logger.warning(f"[{self.provider_name.upper()}] Repetitive content detected")
                return False
