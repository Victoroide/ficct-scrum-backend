...
class LLMProxyService:
    ...
    def __init__(self, enable_fallback: bool = True):
        ...
        logger.info(f"[LLM PROXY] Initialized with fallback chain: {self.fallback_chain}")
    ...
    def generate(
        ...
        **kwargs
        ...
        """
        use_fallback = fallback_enabled if fallback_enabled is not None else self.enable_fallback
        
        logger.info(f"[LLM PROXY] Starting generation: task={task_type}, fallback={use_fallback}")
        
        # Build list of providers to try
        providers_to_try = self.fallback_chain if use_fallback else [self.fallback_chain[0]]
        ...
        for provider_key in providers_to_try:
            ...
            if provider_key not in self.providers:
                logger.warning(f"[LLM PROXY] Provider {provider_key} not available, skipping")
                attempts.append({
            ...
            try:
                ...
                if provider._validate_response(response):
                    ...
                    logger.warning(f"[LLM PROXY] Invalid response from {provider_key}, trying fallback")
                    attempts.append({
                        "provider": provider_key,
                ...
                # Unexpected error
                logger.exception(f"[LLM PROXY] Unexpected error with {provider_key}: {str(e)}")
                attempts.append({
                    "provider": provider_key,
                    "status": "exception",
                    "error": str(e)
        ...
        logger.error(
            f"[LLM PROXY] FAILED - All providers failed:\n{error_summary}"
        )
