...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--mode", type=str, choices=["audit", "cleanup", "sync"], default="audit")
        parser.add_argument("--namespace", type=str, choices=["issues", "sprints", "project_context", "team_members", "all"], default="all")
        ...
        parser.add_argument("--rate-limit", type=int, default=10, help="Max API calls per second (default: 10)")
        parser.add_argument("--sequential", action="store_true", help="Process sequentially instead of parallel (slower but safer)")
        parser.add_argument("--force", action="store_true", help="Force reindex all vectors, ignore content hash checks")
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"Index: {getattr(settings, 'PINECONE_INDEX_NAME', 'N/A')}")
            self.stdout.write()
        except Exception as e:
            raise CommandError(f"Failed to connect to Pinecone: {e}")
        
        # Collect inventories
        db_data = self._collect_database_inventory(options["workspace"], options["namespace"])
    ...
    def _collect_database_inventory(self, workspace_filter, namespace_filter):
        ...
        if namespace_filter in ["all", "issues"]:
            data["issues"] = list(Issue.objects.filter(**filters if filters else {}).select_related("project"))
        
        if namespace_filter in ["all", "sprints"]:
            data["sprints"] = list(Sprint.objects.filter(**filters if filters else {}).select_related("project"))
        ...
        if namespace_filter in ["all", "team_members"]:
            mfilters = {"project__workspace_id": workspace_filter} if workspace_filter else {}
            # Store as (project_id, user_id) tuples to match vector_id format: member_{project_id}_{user_id}
            memberships = ProjectTeamMember.objects.filter(**mfilters).values_list("project_id", "user_id")
        ...
        try:
            ...
            for pinecone_ns, data_key in namespace_map.items():
                if pinecone_ns in namespaces_stats:
                    ...
                    if self.verbose:
                        self.stdout.write(f"[DEBUG] Namespace {pinecone_ns}: {count} vectors")
                    data[data_key] = count
                    
        except Exception as e:
            if self.verbose:
                self.stdout.write(self.style.WARNING(f"[WARNING] Failed to get Pinecone stats: {e}"))
    ...
    def _calculate_costs(self, missing):
        ...
        for ns, records in missing.items():
            ...
            cost = (Decimal(total_tokens) / Decimal(1000)) * OPENAI_EMBEDDING_COST_PER_1K_TOKENS
            
            cost_data[ns] = {"count": len(records), "avg_chars": avg_chars, "total_tokens": total_tokens, "cost": cost}
    ...
    def _display_full_report(self, db_data, pc_data, analysis):
        ...
        # Orphaned
        self.stdout.write(self.style.WARNING("ORPHANED VECTORS (in Pinecone, not in DB):"))
        ...
        if total_orphaned > 0:
            self.stdout.write(self.style.WARNING("  Action: DELETE from Pinecone (free operation)"))
        self.stdout.write()
        
        # Missing
        self.stdout.write(self.style.WARNING("MISSING VECTORS (in DB, not in Pinecone):"))
        for ns in ["issues", "sprints", "projects", "team_members"]:
            ...
            if ns in analysis["cost"] and analysis["cost"][ns]["count"] > 0:
                c = analysis["cost"][ns]
                self.stdout.write(f"    Average text length: {c['avg_chars']} characters")
        ...
        if total_duplicates > 0:
            self.stdout.write(self.style.ERROR("[WARNING] DUPLICATE/EXCESS VECTORS DETECTED:"))
            for ns in ["issues", "sprints", "projects", "team_members"]:
                dup_count = analysis["duplicates"][ns]
                if dup_count > 0:
                    self.stdout.write(f"  {ns}: {dup_count} excess vectors (duplicates or orphans)")
            self.stdout.write(f"\n  Total: {total_duplicates} excess vectors")
            self.stdout.write(self.style.ERROR("  [CRITICAL] This indicates duplicates were created during sync!"))
        ...
        if total_duplicates > 0:
            self.stdout.write(f"{rec_num}. [URGENT] Clean up {total_duplicates} duplicate vectors:")
            self.stdout.write("   Option A: Nuclear cleanup (delete all and resync):")
            self.stdout.write("     python manage.py cleanup_pinecone_duplicates --nuclear --confirm")
            self.stdout.write("   Option B: Manual cleanup per namespace:")
            self.stdout.write("     # Delete issues namespace")
            self.stdout.write("     python manage.py shell")
            self.stdout.write("     >>> from apps.ai_assistant.services.rag_service import RAGService")
            self.stdout.write("     >>> rag = RAGService()")
            self.stdout.write("     >>> rag.pinecone.index.delete(delete_all=True, namespace='issues')")
            self.stdout.write("     # Then resync")
            self.stdout.write("     python manage.py sync_pinecone_vectors --mode sync --namespace issues --confirm")
            rec_num += 1
        
        if total_orphaned > 0:
            self.stdout.write(f"{rec_num}. Run cleanup to remove {total_orphaned} orphaned vectors (free)")
            self.stdout.write("   " + self.style.SUCCESS("python manage.py sync_pinecone_vectors --mode cleanup --confirm"))
            rec_num += 1
        
        if total_missing > 0:
            self.stdout.write(f"{rec_num}. Sync missing vectors (costs ${total_cost:.7f})")
            self.stdout.write("   " + self.style.SUCCESS("python manage.py sync_pinecone_vectors --mode sync --confirm"))
            rec_num += 1
        
        if total_duplicates == 0 and total_orphaned == 0 and total_missing == 0:
            self.stdout.write("  [OK] No action needed - database and Pinecone are in sync!")
    ...
    def _execute_cleanup(self, analysis, dry_run, confirm):
        ...
        if total_orphaned == 0:
            self.stdout.write(self.style.SUCCESS("[OK] No orphaned vectors to clean up"))
            return
        
        if not confirm:
            self.stdout.write(self.style.ERROR("[ERROR] Cleanup requires --confirm flag"))
            return
        
        if dry_run:
            self.stdout.write(self.style.WARNING(f"[DRY RUN] Would delete {total_orphaned} vectors"))
            return
        ...
        for ns_key, vector_ids in analysis["orphaned"].items():
            if vector_ids:
                ns_map = {"issues": "issues", "sprints": "sprints", "projects": "project_context", "team_members": "team_members"}
                ns = ns_map[ns_key]
        ...
        self.stdout.write(self.style.SUCCESS(f"[OK] Deleted {deleted} orphaned vectors"))

    def _execute_sync(self, analysis, dry_run, confirm, workers=8, rate_limit=10, sequential=False, force=False):
        """Sync missing vectors using existing RAGService with parallel processing."""
        total_missing = sum(len(v) for v in analysis["missing"].values())
        
        if total_missing == 0:
            self.stdout.write(self.style.SUCCESS("[OK] All records are already vectorized"))
            return
        ...
        total_cost = sum(c["cost"] for c in analysis["cost"].values())
        self.stdout.write(self.style.WARNING(f"This will cost approximately ${total_cost:.7f}"))
        
        if dry_run:
            self.stdout.write(self.style.WARNING(f"[DRY RUN] Would vectorize {total_missing} records"))
            return
        ...
        if analysis["missing"]["issues"]:
            self.stdout.write(f"\n[1/4] Syncing Issues namespace ({len(analysis['missing']['issues'])} records)...")
            synced, failed = self._sync_issues(analysis["missing"]["issues"], actual_workers, rate_limit, force)
            total_synced += synced
            total_failed += failed
            self.stdout.write(self.style.SUCCESS(f"  [OK] Issues: {synced} synced, {failed} failed"))
        
        # Sync sprints
        if analysis["missing"]["sprints"]:
            self.stdout.write(f"\n[2/4] Syncing Sprints namespace ({len(analysis['missing']['sprints'])} records)...")
            synced, failed = self._sync_sprints(analysis["missing"]["sprints"], actual_workers, rate_limit, force)
            total_synced += synced
            total_failed += failed
            self.stdout.write(self.style.SUCCESS(f"  [OK] Sprints: {synced} synced, {failed} failed"))
        
        # Sync projects
        if analysis["missing"]["projects"]:
            self.stdout.write(f"\n[3/4] Syncing Projects namespace ({len(analysis['missing']['projects'])} records)...")
            synced, failed = self._sync_projects(analysis["missing"]["projects"], actual_workers, rate_limit, force)
            total_synced += synced
            total_failed += failed
            self.stdout.write(self.style.SUCCESS(f"  [OK] Projects: {synced} synced, {failed} failed"))
        
        # Sync team members
        if analysis["missing"]["team_members"]:
            self.stdout.write(f"\n[4/4] Syncing Team Members namespace ({len(analysis['missing']['team_members'])} records)...")
            synced, failed = self._sync_team_members(analysis["missing"]["team_members"], actual_workers, rate_limit, force)
            total_synced += synced
            total_failed += failed
            self.stdout.write(self.style.SUCCESS(f"  [OK] Team Members: {synced} synced, {failed} failed"))
        ...
        self.stdout.write(f"Success rate: {(total_synced/(total_synced+total_failed)*100):.1f}%" if (total_synced+total_failed) > 0 else "N/A")
        self.stdout.write("="*80 + "\n")
    ...
    def _sync_issues(self, issue_ids, workers, rate_limit, force=False):
        ...
        def sync_single_issue(issue):
            ...
            try:
                ...
                # Vectorize issue
                success, error_msg = self.rag_service.index_issue(str(issue.id), force_reindex=force)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(sync_single_issue, issue): issue for issue in issue_ids}
            ...
            for future in as_completed(futures):
                ...
                if completed % 10 == 0 or completed == len(issue_ids):
                    self.stdout.write(f"    Progress: {completed}/{len(issue_ids)} ({stats['synced']} success, {stats['failed']} failed)")
    ...
    def _sync_sprints(self, sprint_ids, workers, rate_limit, force=False):
        ...
        # Parallel processing
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(sync_single_sprint, sprint): sprint for sprint in sprint_ids}
            ...
            for future in as_completed(futures):
                ...
                if completed % 5 == 0 or completed == len(sprint_ids):
                    self.stdout.write(f"    Progress: {completed}/{len(sprint_ids)} ({stats['synced']} success, {stats['failed']} failed)")
    ...
    def _sync_projects(self, project_ids, workers, rate_limit, force=False):
        ...
        def sync_single_project(project):
            ...
            try:
                ...
                # Vectorize project
                success, error_msg = self.rag_service.index_project_context(str(project.id))
        ...
        # Parallel processing
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(sync_single_project, project): project for project in project_ids}
            ...
            for future in as_completed(futures):
                ...
                if completed % 5 == 0 or completed == len(project_ids):
                    self.stdout.write(f"    Progress: {completed}/{len(project_ids)} ({stats['synced']} success, {stats['failed']} failed)")
    ...
    def _sync_team_members(self, team_member_tuples, workers, rate_limit, force=False):
        ...
        def sync_single_member(member_tuple):
            ...
            try:
                ...
                success, error_msg = self.rag_service.index_team_member(str(project_id), int(user_id))
                ...
                with lock:
                    if success:
                        ...
                        stats['failed'] += 1
                        if self.verbose:
                            self.stderr.write(f"  Failed ({project_id}, {user_id}): {error_msg}")
                ...
                with lock:
                    stats['failed'] += 1
                    if self.verbose:
                        self.stderr.write(f"  Exception ({project_id}, {user_id}): {str(e)}")
                return False, (project_id, user_id), str(e)
        
        # Parallel processing
        with ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(sync_single_member, member): member for member in team_member_tuples}
            ...
            for future in as_completed(futures):
                ...
                if completed % 5 == 0 or completed == len(team_member_tuples):
                    self.stdout.write(f"    Progress: {completed}/{len(team_member_tuples)} ({stats['synced']} success, {stats['failed']} failed)")
