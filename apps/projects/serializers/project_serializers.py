...
class ProjectSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'estimated_hours', 'budget', 'project_settings', 'is_active',
            'created_by', 'team_member_count', 'issue_count',
    ...
    def validate_key(self, value):
        ...
        if self.instance:
            if Project.objects.exclude(id=self.instance.id).filter(
                ...
                raise serializers.ValidationError("Project with this key already exists in workspace")
        else:
    ...
    def create(self, validated_data):
        ...
        default_issue_types = [
            ...
            {'name': 'Improvement', 'category': 'improvement', 'color': '#2684FF', 'icon': 'improvement'},
        ]
        ...
        default_statuses = [
            {'name': 'To Do', 'category': 'to_do', 'color': '#DFE1E6', 'order': 1, 'is_initial': True},
            {'name': 'In Progress', 'category': 'in_progress', 'color': '#0052CC', 'order': 2},
...
class ProjectTeamMemberSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'project', 'user', 'role', 'permissions',
            'hourly_rate', 'is_active', 'joined_at', 'updated_at'
        ]
...
class IssueTypeSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if self.instance:
            if IssueType.objects.exclude(id=self.instance.id).filter(
                project=project,
                name=value
            ).exists():
                raise serializers.ValidationError("Issue type with this name already exists in project")
        else:
...
class WorkflowStatusSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'project', 'name', 'category', 'description',
            'color', 'order', 'is_initial', 'is_final', 'is_active',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'project', 'created_at', 'updated_at']
    ...
    def validate_name(self, value):
        project = self.context.get('project')
        if self.instance:
            if WorkflowStatus.objects.exclude(id=self.instance.id).filter(
                project=project,
                name=value
            ).exists():
                raise serializers.ValidationError("Status with this name already exists in project")
        else:
...
class WorkflowTransitionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'project', 'name', 'from_status', 'to_status',
            'conditions', 'validators', 'post_functions', 'is_active',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'project', 'created_at', 'updated_at']
...
class AddTeamMemberSerializer(serializers.Serializer):
    ...
    role = serializers.ChoiceField(choices=ProjectTeamMember.ROLE_CHOICES, default='developer')
