...
class AngularAnalyzer:
    ...
    MODULE_PATTERN = re.compile(
        r'@NgModule\s*\(\s*\{([^}]+)\}\s*\)\s*export\s+class\s+(\w+)',
        re.DOTALL
    )
    ...
    ROUTE_PATTERN = re.compile(
        r'RouterModule\.for(?:Root|Child)\s*\(\s*\[([^\]]+)\]',
        re.DOTALL
    )
    ...
    def _analyze_component(self, file_path: str, content: str):
        ...
        for metadata_str, class_name in matches:
            ...
            template_match = re.search(r'template(?:Url)?:\s*[\'"]([^\'"]+)[\'"]', metadata_str)
    ...
    def _analyze_service(self, file_path: str, content: str):
        ...
        for metadata_str, class_name in matches:
            # Extract providedIn
            provided_in_match = re.search(r'providedIn:\s*[\'"]([^\'"]+)[\'"]', metadata_str)
        ...
        for routes_str in matches:
            ...
            for path in route_matches:
                ...
                lazy_pattern = re.compile(
                    re.escape(path) + r'[^}]*loadChildren:\s*\(\)\s*=>\s*import\([\'"]([^\'"]+)[\'"]',
                    re.DOTALL
                )
    ...
    def _extract_dependencies(self, content: str, class_name: str) -> List[str]:
        ...
        class_pattern = r'export\s+class\s+' + re.escape(class_name) + r'[^{]*\{(.+?)\n\s*\}'
