...
class GitHubCodeFetcher:
    ...
    def fetch_repo_tree(self, use_cache: bool = True) -> List[Dict]:
        ...
        if use_cache:
            ...
            if cached:
                logger.info(f"Using cached repository tree for {self.integration.repository_name}")
                return cached
        ...
        logger.info(f"Fetching repository tree from GitHub: {self.integration.repository_name}")
    ...
    def fetch_multiple_files(self, file_paths: List[str], max_files: int = 100) -> Dict[str, str]:
        """
