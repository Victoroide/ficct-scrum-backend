...
class DiagramDataService:
    ...
    def get_workflow_data(self, project) -> Dict:
        ...
        statuses = WorkflowStatus.objects.filter(
            project=project
        ).prefetch_related('issues').order_by('order', 'name')
        ...
        canvas_width = max(
            1600,
            start_x * 2 + (len(nodes) * horizontal_spacing)
        )
    ...
    def get_dependency_data(self, project, filters: Optional[Dict] = None) -> Dict:
        ...
        for issue in issues:
            ...
            node = {
                ...
                'status_color': self._get_status_color(issue.status.category) if issue.status else '#5E6C84',
        ...
        logger.debug(f"Querying IssueLinks where source or target in {len(issue_uuid_list)} issues")
        
        links = IssueLink.objects.filter(
            Q(source_issue_id__in=issue_uuid_list) | Q(target_issue_id__in=issue_uuid_list),
        ...
        if link_count == 0:
            ...
            logger.warning(f"No links found for filtered issues. Total links in project: {total_links}")
        
        for link in links:
            source_id = str(link.source_issue.id)
            target_id = str(link.target_issue.id)
            
            logger.debug(f"Processing link: {link.source_issue.key} {link.link_type} {link.target_issue.key}")
            ...
            if source_id in issue_ids and target_id in issue_ids:
                ...
                logger.debug(f"Skipped link (nodes not in filtered set): {source_id} -> {target_id}")
        ...
        if len(edges) == 0 and len(nodes) > 0:
            ...
            metadata['debug'] = {
                ...
                'message': 'No dependencies found between filtered issues' if total_project_links > 0 else 'No issue links exist in this project yet'
            }
