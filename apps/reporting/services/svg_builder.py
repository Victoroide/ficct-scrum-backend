...
def create_rect(x: float, y: float, width: float, height: float,
                fill: str, stroke: Optional[str] = None, stroke_width: int = 2,
                radius: int = 6, opacity: float = 1.0, 
                shadow: bool = False, css_class: str = '') -> str:
    """
...
def create_circle(cx: float, cy: float, r: float, fill: str,
                  stroke: Optional[str] = None, stroke_width: int = 2) -> str:
    """
    Create a circle.
...
def create_ellipse(cx: float, cy: float, rx: float, ry: float, fill: str,
...
def create_text(x: float, y: float, text: str, 
...
def create_line(x1: float, y1: float, x2: float, y2: float,
                stroke: Optional[str] = None, width: int = 2,
                dashed: bool = False, marker_end: Optional[str] = None,
...
def create_arrow(x1: float, y1: float, x2: float, y2: float,
                 curve: bool = True, arrow_type: str = 'default',
                 stroke: Optional[str] = None, width: int = 2) -> str:
    """
    ...
    if curve and abs(x2 - x1) > 50:
        ...
        # Straight line
        return create_line(x1, y1, x2, y2, stroke=stroke, width=width, marker_end=marker)


def create_path(d: str, fill: str = 'none', stroke: Optional[str] = None,
...
def create_node_box(x: float, y: float, width: float, height: float,
                    title: str, subtitle: Optional[str] = None,
                    badge: Optional[str] = None, color: Optional[str] = None,
...
def create_legend(x: float, y: float, items: List[Tuple[str, str]],
...
def create_grid(x: float, y: float, width: float, height: float,
                x_lines: List[float], y_lines: List[float]) -> str:
    """
...
def create_axes(x: float, y: float, width: float, height: float,
    ...
    if y_labels:
        ...
        for i, label in enumerate(y_labels):
            label_y = y + height - (i * label_spacing) if len(y_labels) > 1 else y + height / 2
            parts.append(create_text(
                x - 10, label_y + 4,
                str(label),
                size=ds.FONTS['size_small'],
