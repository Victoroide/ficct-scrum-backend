...
from .models import SystemLog, ErrorLog, AuditLog, Alert, AlertRule
...
class LoggerService:
    ...
    @staticmethod
    @transaction.atomic
    def log_info(action: str, user: Optional[User] = None, ip_address: Optional[str] = None, 
                 details: Optional[Dict[str, Any]] = None, content_object: Any = None):
        try:
    ...
    @staticmethod
    @transaction.atomic
    def log_warning(action: str, user: Optional[User] = None, ip_address: Optional[str] = None,
                    details: Optional[Dict[str, Any]] = None, content_object: Any = None):
        try:
            SystemLog.objects.create(
                level='WARNING',
    ...
    @staticmethod
    @transaction.atomic
    def log_error(action: str, error: str, user: Optional[User] = None, 
    ...
    @staticmethod
    @transaction.atomic
    def log_critical(action: str, error: str, user: Optional[User] = None,
                     ip_address: Optional[str] = None, details: Optional[Dict[str, Any]] = None,
                     content_object: Any = None):
        try:
            stack_trace = traceback.format_exc()
    ...
    @staticmethod
    @transaction.atomic
    def log_api_request(method: str, path: str, user: Optional[User] = None,
    ...
    @staticmethod
    @transaction.atomic
    def log_audit(user: User, action: str, resource_type: str, content_object: Any = None,
    ...
    @staticmethod
    @transaction.atomic
    def log_security_event(action: str, user: Optional[User] = None,
        ...
        try:
            ...
            LoggerService._check_alert_rules('security_event', {
                'action': action,
                'user': user,
                'ip_address': ip_address,
                'severity': severity
    ...
    def _evaluate_alert_condition(rule: AlertRule, data: Dict[str, Any]) -> bool:
        try:
            ...
            if rule.condition_type == 'error_count':
                ...
                return severity_levels.get(event_severity, 1) >= severity_levels.get(severity_threshold, 2)
    ...
    def _trigger_alert(rule: AlertRule, data: Dict[str, Any]):
        try:
            ...
            if rule.condition_type == 'error_count':
                ...
                message += f" - Security event detected: {data.get('action', 'Unknown')}"
    ...
    def get_system_health_metrics():
        try:
            ...
            return {
                'total_logs_24h': SystemLog.objects.filter(created_at__gte=last_24h).count(),
