...
def workspace_asset_path(instance, filename):
    return f"workspaces/workspace_{instance.id}/assets/{filename}"
...
class Workspace(models.Model):
    ...
    workspace_type = models.CharField(
        max_length=20, choices=WORKSPACE_TYPE_CHOICES, default="general"
    )
    visibility = models.CharField(
        max_length=20, choices=VISIBILITY_CHOICES, default="private"
    )
    cover_image = models.ImageField(
        upload_to=workspace_asset_path, null=True, blank=True
    )
