...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--all", action="store_true", help="Reindex all projects")
        ...
        parser.add_argument(
            "--issues-only", action="store_true", help="Only reindex issues"
        )

        parser.add_argument(
            "--sprints-only", action="store_true", help="Only reindex sprints"
        )

        parser.add_argument(
            "--members-only", action="store_true", help="Only reindex team members"
        )

        parser.add_argument(
            "--projects-only", action="store_true", help="Only reindex project contexts"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not rag_service.available:
                self.stdout.write(
                    self.style.ERROR(
                        f"RAG service unavailable: {rag_service.error_message}"
                    )
                )
                return
        ...
        index_issues = not any(
            [options["sprints_only"], options["members_only"], options["projects_only"]]
        )

        index_sprints = not any(
            [options["issues_only"], options["members_only"], options["projects_only"]]
        )

        index_members = not any(
            [options["issues_only"], options["sprints_only"], options["projects_only"]]
        )

        index_projects = not any(
            [options["issues_only"], options["sprints_only"], options["members_only"]]
        )

        # Clear if requested
        if options["clear_first"]:
            self._clear_namespaces(
                rag_service, index_issues, index_sprints, index_members, index_projects
            )
        ...
        for i, project in enumerate(projects, 1):
            ...
            self.stdout.write(
                self.style.HTTP_INFO(
                    f"[{i}/{len(projects)}] Project: {project.name} ({project.key})"
                )
            )
    ...
    def _index_project_issues(self, rag_service, project):
        ...
        try:
            ...
            self.stdout.write(
                f"    {self.style.SUCCESS('✓')} {result['indexed']}/{result['total']}"
                f" indexed ({result['success_rate']}%)"
            )
        ...
        try:
            ...
            for member in members:
                success, error = rag_service.index_team_member(
                    project_id=str(project.id), user_id=member.user_id
                )
    ...
    def _index_project_context(self, rag_service, project):
        ...
        try:
            success, error = rag_service.index_project_context(
