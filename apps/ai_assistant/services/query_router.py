...
class QueryRouter:
    ...
    PRIORITY_KEYWORDS = [
        "priority", "critical", "urgent", "high", "low", "p1", "p2", "p3", "p4",
        "importante", "urgente", "critico"
    ]
    
    MEMBER_KEYWORDS = [
        "who", "assignee", "assigned", "working on", "member", "team",
        "quien", "asignado", "trabajando", "miembro", "equipo"
    ]
    
    SPRINT_KEYWORDS = [
        "sprint", "iteration", "current", "active", "this week", "this month",
        "iteracion", "actual", "activo", "esta semana", "este mes"
    ]
    
    STATUS_KEYWORDS = [
        "status", "done", "completed", "in progress", "todo", "backlog",
        "estado", "hecho", "completado", "en progreso", "pendiente"
    ]
    
    TEMPORAL_KEYWORDS = [
        "recent", "recently", "latest", "last", "today", "yesterday",
        "reciente", "ultimo", "hoy", "ayer", "recientemente"
    ]
    ...
    def classify_query_intent(self, query: str) -> str:
        ...
        if re.search(name_pattern, query):
            logger.info(f"[ROUTER] Detected person name, classified as member_query: '{query}'")
            return "member_query"
    ...
    def _build_priority_strategy(self, query_lower: str, project_id: Optional[str]) -> Dict:
        """Build search strategy for priority-based queries."""
        
        # Detect priority level
        priority_filters = []
        if any(word in query_lower for word in ["critical", "urgent", "p1", "critico", "urgente"]):
    ...
    def _build_member_strategy(self, query: str, query_lower: str, project_id: Optional[str]) -> Dict:
        ...
        logger.debug(f"[ROUTER] Member strategy: names={names}, namespaces={namespaces}")
    ...
    def _build_sprint_strategy(self, query_lower: str, project_id: Optional[str]) -> Dict:
        ...
        if any(word in query_lower for word in ["current", "active", "this", "actual", "activo"]):
            filters["status"] = "active"
        
        # Multi-namespace: sprints + issues in sprint
        namespaces = ["sprints", "issues"]
        
        logger.debug(f"[ROUTER] Sprint strategy: filters={filters}, namespaces={namespaces}")
    ...
    def _build_status_strategy(self, query_lower: str, project_id: Optional[str]) -> Dict:
        ...
        if any(word in query_lower for word in ["done", "completed", "hecho", "completado"]):
            filters["status_category"] = "done"
        elif any(word in query_lower for word in ["in progress", "working", "en progreso", "trabajando"]):
    ...
    def _build_temporal_strategy(self, query_lower: str, project_id: Optional[str]) -> Dict:
        ...
        logger.debug(f"[ROUTER] Temporal strategy: threshold={threshold}, filters={filters}")
