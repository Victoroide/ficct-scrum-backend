...
ISSUE_SEMANTIC_FIELDS = {
    'title', 'description', 'labels', 'acceptance_criteria', 
    'status_id', 'priority', 'assignee_id', 'sprint_id', 'issue_type_id'
}

SPRINT_SEMANTIC_FIELDS = {
...
def auto_index_issue(sender, instance, created, **kwargs):
    ...
    if not created and hasattr(instance, '_old_values'):
        ...
        for field in ISSUE_SEMANTIC_FIELDS:
            ...
            if old_val != new_val:
                logger.debug(f"Issue {instance.id} field '{field}' changed: {old_val} -> {new_val}")
                should_reindex = True
                break
...
def auto_index_sprint(sender, instance, created, **kwargs):
    ...
    if not created and hasattr(instance, '_old_values'):
        ...
        for field in SPRINT_SEMANTIC_FIELDS:
            ...
            new_val = getattr(instance, field, None)
            if old_val != new_val:
                logger.debug(f"Sprint {instance.id} field '{field}' changed: {old_val} -> {new_val}")
                should_reindex = True
                break
