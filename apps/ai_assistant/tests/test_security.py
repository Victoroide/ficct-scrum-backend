...
class TestAISecurityIsolation:
    ...
    def setup_method(self):
        ...
        self.project_a = ProjectFactory(
            workspace=self.workspace, name="Project A", key="PROJA"
        )
        ProjectTeamMember.objects.create(
            project=self.project_a, user=self.alice, role="member", is_active=True
        )

        # Project B - Bob is member
        self.project_b = ProjectFactory(
            workspace=self.workspace, name="Project B", key="PROJB"
        )
        ProjectTeamMember.objects.create(
            project=self.project_b, user=self.bob, role="member", is_active=True
        )
    ...
    def test_workspace_member_can_access_all_projects(self, api_client):
        ...
        WorkspaceMember.objects.create(
            workspace=self.workspace, user=admin, role="admin", is_active=True
        )
        ...
        data_a = {"query": "test", "project_id": str(self.project_a.id)}
        ...
        data_b = {"query": "test", "project_id": str(self.project_b.id)}
...
class TestPineconeFilterSecurity:
    ...
    def setup_method(self):
        ...
        ProjectTeamMember.objects.create(
            project=self.project, user=self.user, role="member", is_active=True
        )
    ...
    def test_pinecone_query_includes_project_filter(
        ...
        rag.semantic_search(query="test query", project_id=project_id, top_k=10)
