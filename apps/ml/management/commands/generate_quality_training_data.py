...
class Command(BaseCommand):
    ...
    def _validate_constraints(self, options: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if str(self.preserve_project.workspace.id) != workspace_uuid:
                raise CommandError("Preserve project does not belong to specified workspace")
    ...
    def _analyze_current_state(self) -> None:
        ...
        existing_sprints = Sprint.objects.filter(project__workspace=self.workspace).count()
        existing_issues = Issue.objects.filter(project__workspace=self.workspace).count()
    ...
    def _create_generation_plan(self, num_projects: int) -> Dict[str, Any]:
        ...
        selected_archetypes = random.sample(
            PROJECT_ARCHETYPES, 
            min(num_projects, len(PROJECT_ARCHETYPES))
        )
        ...
        self.stdout.write(f"Timeline: {plan['timeline_start'].date()} to {plan['timeline_end'].date()}")
        self.stdout.write()
    ...
    def _show_dry_run_summary(self, plan: Dict[str, Any]) -> None:
        ...
        self.stdout.write(self.style.SUCCESS(
            f"    --preserve-project {self.preserve_project.id} \\"
        ))
        self.stdout.write(self.style.SUCCESS(
            "    --confirm"
        ))
    ...
    def _show_post_generation_guide(self) -> None:
        ...
        self.stdout.write(self.style.SUCCESS("  .venv\\Scripts\\python.exe analyze_workspace_data.py"))
        self.stdout.write()
        ...
        self.stdout.write(f"  >>> projects = Project.objects.filter(workspace_id='{self.workspace.id}')")
        self.stdout.write("  >>> for p in projects: print(f'{p.key}: {p.name} - {p.issues.count()} issues')")
        self.stdout.write()
        
        self.stdout.write(self.style.SUCCESS("Step 3: Check Completed Issues with Effort"))
        self.stdout.write("  Verify effort data was logged:")
        self.stdout.write(self.style.SUCCESS("  python manage.py shell"))
        self.stdout.write("  >>> from apps.projects.models import Issue")
        self.stdout.write(f"  >>> completed = Issue.objects.filter(project__workspace_id='{self.workspace.id}', status__is_final=True, actual_hours__isnull=False)")
        self.stdout.write("  >>> print(f'Completed issues with effort: {completed.count()}')")
        self.stdout.write("  >>> print(f'Average effort: {completed.aggregate(avg=models.Avg(\"actual_hours\"))[\"avg\"]:.2f} hours')")
        self.stdout.write()
        
        self.stdout.write(self.style.SUCCESS("Step 4: Train Effort Prediction Model"))
        self.stdout.write("  Copy-paste command:")
        self.stdout.write(self.style.SUCCESS("  python manage.py train_ml_model effort_prediction"))
        self.stdout.write()
        self.stdout.write("  Expected output:")
        ...
        self.stdout.write("  Copy-paste command:")
        self.stdout.write(self.style.SUCCESS("  python manage.py train_ml_model story_points"))
        self.stdout.write()
        self.stdout.write("  Expected output:")
        ...
        self.stdout.write("  Copy-paste command:")
        self.stdout.write(self.style.SUCCESS("  python manage.py list_ml_models --active-only"))
        self.stdout.write()
        self.stdout.write("  Expected: See effort_prediction and story_points models with 'active' status")
        self.stdout.write()
        
        self.stdout.write(self.style.SUCCESS("Step 7: Test Prediction"))
        self.stdout.write("  Run a test prediction to verify model works:")
        self.stdout.write(self.style.SUCCESS("  python manage.py shell"))
        self.stdout.write("  >>> from apps.ml.services.prediction_service import PredictionService")
        ...
        if self.generated_projects:
            ...
            self.stdout.write(f"  ... )")
            self.stdout.write("  >>> print(f\"Predicted: {result['predicted_hours']} hours\")")
            self.stdout.write("  >>> print(f\"Confidence: {result['confidence']:.2%}\")")
        ...
        self.stdout.write(self.style.SUCCESS("  python manage.py detect_anomalies --all"))
        self.stdout.write()
    ...
    def _disconnect_vectorization_signals(self) -> List[Tuple]:
        ...
        try:
            ...
            disconnected.append(('post_delete', ai_signals.remove_issue_from_index, Issue))
            ...
            disconnected.append(('pre_save', ai_signals.store_sprint_old_values, Sprint))
            
            post_delete.disconnect(ai_signals.remove_sprint_from_index, sender=Sprint)
            disconnected.append(('post_delete', ai_signals.remove_sprint_from_index, Sprint))
            
            if self.verbose_logging:
                self.stdout.write("[DEBUG] Disconnected 6 Pinecone auto-indexing signals")
