...
ROLE_SKILLS = {
    "Senior Backend Engineer": ["Python", "Django", "PostgreSQL", "Redis", "AWS", "API Design"],
...
def generate_team_members_impl(command) -> None:
    ...
    ROLE_CHARACTERISTICS = {
        "Senior Backend Engineer": {"velocity_factor": 1.3, "estimation_accuracy": 0.85},
    ...
    team_profiles = [
        {"first_name": "Sarah", "last_name": "Chen", "email": "sarah.chen@ficct.com", "role": "Senior Backend Engineer"},
        {"first_name": "Michael", "last_name": "Rodriguez", "email": "michael.rodriguez@ficct.com", "role": "Frontend Developer"},
        {"first_name": "Emily", "last_name": "Johnson", "email": "emily.johnson@ficct.com", "role": "Full Stack Developer"},
        {"first_name": "David", "last_name": "Kim", "email": "david.kim@ficct.com", "role": "QA Engineer"},
        {"first_name": "Jessica", "last_name": "Martinez", "email": "jessica.martinez@ficct.com", "role": "DevOps Engineer"},
        {"first_name": "Alex", "last_name": "Thompson", "email": "alex.thompson@ficct.com", "role": "UI/UX Designer"},
        {"first_name": "Ryan", "last_name": "Patel", "email": "ryan.patel@ficct.com", "role": "Junior Developer"},
    ]
...
def create_projects_impl(command, plan: Dict[str, Any]) -> None:
    ...
    for archetype in plan["projects"]:
        ...
        for role_name, member_data in command.team_members.items():
            ...
            elif archetype["key"] in ["MOBILE", "ADMIN"] and role_name == "Frontend Developer":
            ...
            elif archetype["key"] in ["PAYMENT", "API"] and role_name == "Senior Backend Engineer":
    ...
    for project in command.generated_projects:
        ...
        for issue_type, count in type_counts.items():
            for _ in range(count):
                ...
                type_templates = templates.get(issue_type, [f"Implement {issue_type} feature"])
                title = random.choice(type_templates) if type_templates else f"Implement {issue_type} for {project.name}"
                ...
                hours_per_point = 6.0
                estimated_hours = Decimal(story_points * hours_per_point * random.uniform(0.8, 1.2))
                ...
                if rand < 0.60 and completed_sprints:
                    ...
                    sprint = random.choice(planned_sprints) if planned_sprints and random.random() < 0.5 else None
                    status_name = "Backlog"
...
def log_effort_data_impl(command) -> None:
    ...
    completed_issues = [
        issue for issue in command.generated_issues
        if issue.status.is_final
    ]
    ...
    for issue in completed_issues:
        ...
        if issue.assignee:
            member_data = next(
                (m for m in command.team_members.values() if m["user"] == issue.assignee),
                None
            )
        ...
        issue.resolved_at = timezone.make_aware(
            datetime.combine(issue.sprint.end_date, datetime.min.time())
        ) if issue.sprint else timezone.now()
        issue.save()
...
def create_dependencies_impl(command) -> None:
    ...
    for project in command.generated_projects:
        ...
        # Group by type
        api_issues = [i for i in project_issues if "API" in i.title or "Backend" in i.title or "endpoint" in i.title.lower()]
        ui_issues = [i for i in project_issues if "UI" in i.title or "Frontend" in i.title or "interface" in i.title.lower()]
        
        # Create "blocks" relationships: API blocks UI
        for ui_issue in ui_issues[:min(5, len(ui_issues))]:  # Limit to 5 dependencies per project
            if api_issues:
...
def inject_anomalies_impl(command) -> None:
    ...
    project_sprints = [s for s in command.generated_sprints if s.project == project and s.status == "completed"]
    if len(project_sprints) >= 3:
        # Last 2 sprints have much lower velocity
        for sprint in project_sprints[-2:]:
            sprint.completed_points = sprint.committed_points * Decimal("0.5")  # Only 50% completed
            sprint.save()
    ...
    if "Senior Backend Engineer" in command.team_members:
        ...
        critical_issues = [i for i in command.generated_issues if i.priority == "P1" and not i.status.is_final]
    ...
    if len(command.generated_projects) >= 3:
        ...
        project_issues = [i for i in command.generated_issues if i.project == project and not i.status.is_final]
    ...
    if len(command.generated_projects) >= 4:
        ...
        epic = Issue.objects.create(
            ...
            sprint=[s for s in command.generated_sprints if s.project == project and s.status == "completed"][0] if [s for s in command.generated_sprints if s.project == project and s.status == "completed"] else None,
            key=str(Issue.objects.filter(project=project).count() + 1),
